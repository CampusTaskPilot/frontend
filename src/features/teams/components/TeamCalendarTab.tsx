...
export function TeamCalendarTab({ teamId, currentUserId, isLeader }: TeamCalendarTabProps) {
  ...
  function handleDeleteRequest(eventIds: string[]) {
    ...
    const preview =
      labels.length > 0
        ? `${labels.join(', ')}${extraCount > 0 ? ` 외 ${extraCount}개` : ''}`
        : `${eventIds.length}개 일정`

    setErrorMessage('')
    setCalendarDeleteIntent({
      eventIds,
      title: eventIds.length === 1 ? '이 일정을 삭제할까요?' : '선택한 일정을 삭제할까요?',
  ...
  return (
    <>
      <div className="space-y-4">
        <Card className="border-campus-200/80 bg-white/90 px-5 py-5 shadow-card sm:px-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <h2 className="text-2xl font-semibold tracking-tight text-campus-900">팀 일정</h2>
                <span className="inline-flex rounded-full border border-campus-200 bg-campus-50 px-3 py-1 text-xs font-medium text-campus-600">
                  {isLeader ? '리더 관리 가능' : '읽기 전용'}
                </span>
              </div>
              <p className="max-w-2xl text-sm leading-6 text-campus-600">
                날짜별 일정을 확인하고 필요한 일정만 간단하게 관리할 수 있습니다.
              </p>
            </div>
        ...
        {!isLeader ? (
          <Card className="border-campus-200 bg-campus-50 py-4">
            <p className="text-sm text-campus-600">
              팀 캘린더는 읽기 전용입니다. 일정 추가, 수정, 삭제는 리더만 할 수 있습니다.
            </p>
          </Card>
        ) : null}
        ...
        {errorMessage && !isModalOpen && !calendarDeleteIntent ? (
          <Card className="border-rose-200 bg-rose-50 py-4">
        ...
        {isLoading ? (
          ...
          <Card className="border-campus-200 bg-white py-10">
            <div className="space-y-3 text-center">
              <h3 className="text-2xl font-semibold text-campus-900">등록된 일정이 없습니다</h3>
              <p className="text-sm text-campus-600">
                아직 팀 일정이 비어 있습니다.
                {isLeader
